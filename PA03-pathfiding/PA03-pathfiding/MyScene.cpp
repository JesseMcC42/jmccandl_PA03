...
bool MyScene::create( std::istream & inputfile)
{
  ...
  m_backdrop=new MyAgent('x', false,false);
  assert(m_backdrop);
  ...
  for (int i = 0; i < y; i++)			// down (row)
  {
    for (int j = 0; j < x; j++)		// across (column)
    {
      ...
      if (rent != NULL)		// it might not be an agent or object
      {
        ...
        if(c=='z') agent = new MyHumanAgent(c, true);
        else if(c=='k') agent = new MyDogAgent(c, true);
        else if(c=='d') agent = new MyCatAgent(c, true);
		else {
			agent = new MyAgent(c, rent->agent);
			if (c == 'b' || c == 'w') {
				items += 1;
			}
		}
		...
		agent->setTrue(j * cell_w, i * cell_h);
...
bool MyScene::createLoader()
{
	...
	//start to create backdrop agent
	string backdropName = "imgs/loading.png";
...
bool MyScene::createWinner()
{
	...
	//start to create backdrop agent
	string backdropName = "imgs/Win.png";
...
bool MyScene::createFailer()
{
	...
	//start to create backdrop agent
	string backdropName = "imgs/Fail.png";
...
bool MySceneManager::createLoad()
{
	MyScene* level = new MyScene();
	assert(level);
	if (!level->createLoader()) return false;
	this->add("Loader", level);
	return true;
}

bool MySceneManager::createWin()
{
	MyScene* level = new MyScene();
	assert(level);
	if (!level->createWinner()) return false;
	this->add("Win", level);
	return true;
}

bool MySceneManager::createFail()
{
	MyScene* level = new MyScene();
	assert(level);
	if (!level->createFailer()) return false;
	this->add("Fail", level);
	return true;
}
